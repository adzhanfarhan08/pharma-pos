    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($username) || empty($email) || empty($password)) {
            ...
            $check = mysqli_query($conn, "SELECT * FROM tb_users WHERE username = '$username' OR email = '$email'");
            if (mysqli_num_rows($check) > 0) {
                ...
                $query = "INSERT INTO tb_users (username, email, password, role_id) VALUES ('$username', '$email', '$password', 2)";
